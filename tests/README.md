...
- `tests/fins/`
  ...
  - `tests/fins/test_cninfo_downloader.py` 负责守住巨潮 discovery 的网络隔离与候选去重边界：测试必须通过 `httpx.MockTransport` 注入全市场 `szse_stock.json` 与 `hisAnnouncement/query` fixture，不访问真实巨潮；公司解析 fixture 必须覆盖深市、沪市与全文搜索噪声不影响精确 ticker 解析；候选 fixture 必须覆盖 `secCode` 与 `adjunctType=PDF` 过滤，并守住摘要、英文版/`（英文）`/英文简版、港股公告、关于财报正本的公告类材料不会进入正本候选；报告去重键必须是 `(fiscal_year, fiscal_period)`，同一年同 period 才按更正版优先、披露日期最新选择，不能把不同 fiscal_year 的年报互相吞掉
  - `tests/fins/test_hkexnews_downloader.py` 负责守住披露易 discovery 的网络隔离与 HK 解析边界：测试必须通过 `httpx.MockTransport` 注入 fixture，不访问真实披露易；`0700/00700/700.HK` 要收敛到 5 位 `STOCK_CODE`，英文入口或英文标题候选不得进入 active，Q1/Q2/Q3/Q4 空结果不得抛异常
  - `tests/fins/test_cn_download_workflow.py` 负责守住 CN/HK 下载 workflow 的仓储顺序与恢复语义：fake downloader 不写 workspace，PDF / Docling JSON 必须经 blob 仓储写入，source meta 必须经 source 仓储 create/update 提交；fast skip、PDF SHA skip、staged PDF/Docling 恢复、按财期区分的默认下载窗口、`download --rebuild` 本地重建、overwrite ticker 级清理，以及 PDF 下载 gate 不覆盖 Docling 转换，都要用文件系统仓储 fixture 验证
  ...
  - 其中 `tests/fins/test_llm_ci_scripts.py` 负责守住 `utils/llm_ci_process.py`、`utils/llm_ci_score.py` 与 `utils/llm_docling_ci_score.py` 的最小增量调度、同一 ticker 子批次串行化、failed filings 识别、SEC form 归一化、CN/HK Docling source/report kind 组合、summary/overall_summary 聚合与输入解析边界，避免 CI 优化辅助脚本在执行层漂移
  - 其中 `tests/fins/test_score_docling_ci.py` 负责守住 CN/HK Docling scorer 的最小闭环：样本发现必须通过 source/processed 仓储对齐 active filing/material，snapshot 读取必须通过 blob 仓储，坏 `tool_snapshot_meta.json` 进入 completeness hard gate，正文表格占位符必须能解引用到现有 `list_tables/get_table`，annual profile 必须识别中文与繁中文财报标题同义词及三大表表述，financial profile 不能仅凭财报行项目判定三大表存在，并且 JSON/MD 报告保持可供后续 CI runner 复用的结构
  - 其中 `tests/fins/test_financial_enhancer_coverage.py` 与 `tests/fins/test_fins_enhanced_processors.py` 共同守住 fins 表格金融语义增强边界：`FinsDoclingProcessor` 可以从 Docling 表体同源证据推断三大报表语义 caption，但 CN/HK 财报语义必须留在 fins 层，不能下沉到 engine `DoclingProcessor`
...
- `tests/services/test_fins_download_lane_gate.py` 负责守住 Service 装配出的 CN/HK PDF 下载段 gate：必须使用可轮询取消的 `try_acquire`，等待 permit 时要响应取消并按节流周期回收 dead-PID stale permit，不能把 Docling 转换包进下载 lane。
...
- `tests/fins/test_ground_truth_baseline.py`、`tests/fins/test_score_sec_ci.py` 与 `tests/fins/test_score_docling_ci.py` 还要继续守住 processed snapshot 边界：离线基线/评分工具只能通过 `ProcessedDocumentRepositoryProtocol` 发现样本、通过 `DocumentBlobRepositoryProtocol` 读取 `tool_snapshot_*.json`，不能在生产代码里回退到直接拼 `workspace/portfolio/{ticker}/processed/...` 或 `glob` 扫描目录。
...
- `test_cli_interactive_coverage.py` 这类 CLI UI 测试里的 fake session 应直接实现 `ChatServiceProtocol` / `PromptServiceProtocol` 公开方法（如 `submit_turn`、`resume_pending_turn`、`list_resumable_pending_turns`、`cleanup_stale_pending_turns`、`submit`），不要继续只靠旧的 `stream/stream_turn` 形态再由测试把对象硬塞进 `interactive_ui`。
